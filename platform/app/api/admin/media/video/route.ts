...
async function POSTHandler(request: Request) {
  ...
  const duration =
    typeof body.duration === 'number' && DURATIONS.has(body.duration)
      ? body.duration
      : undefined;
  const aspectRatio =
  ...
  try {
    ...
    await db
      ...
      .values({
        ...
        actorId: user.userId,
        actorType: 'user',
        action: 'media.video_submitted',
        entityType: 'media',
        entityId: job.id,
        details: { referenceCount: referenceUrls.length },
      })
      .catch(() => null);
    await logAgentRun({
      agentName: 'Creative Studio (video)',
      channel: 'admin',
      status: 'success',
      outcome: `Job submitted: ${prompt.slice(0, 140)}`,
    ...
    await logAgentRun({
      ...
      channel: 'admin',
      status: 'error',
      outcome: 'Job submission failed',
      durationMs: Date.now() - startedAt,
    });
    return NextResponse.json(
      { error: 'The video job could not be started right now.' },
      { status: 502 },
    );
  }
}

export const POST = withRequestLog('/api/admin/media/video', POSTHandler);
