...
import {
  AdminProjectList,
  type Project,
} from '@/components/admin-project-list';
...
export default async function AdminProjectsPage() {
  ...
  try {
    ...
    activeProjects = rows.rows.map((row) => {
      ...
      const billing: Billing = !invoiced
        ? 'none'
        : outstanding > 0
          ? 'awaiting'
          : 'paid';
      return {
        ...
        nextGate: typeof record.next_gate === 'string' ? record.next_gate : '',
        billing,
      };
    });
  } catch {
      ...
      <div className="admin-layout">
        <section className="admin-panel projects-panel">
          <div className="panel-head">
            <div>
              <small>LATEST 30 PROJECTS</small>
              <h2>
                {loadFailed ? 'Records unavailable' : 'Delivery and billing'}
              </h2>
            </div>
            <CheckCircle2 />
          </div>
          {loadFailed ? (
            <div className="empty-admin" role="alert">
              ...
              <span>Check the database connection, then reload this page.</span>
            </div>
          ) : activeProjects.length ? (
            <AdminProjectList initialProjects={activeProjects} />
          ) : (
            <div className="empty-admin">
              <strong>No projects created yet.</strong>
              <span>
                Create a project once there is a real client, objective and
                agreed next step.
              </span>
            </div>
          )}
