...
export async function loadProspecting() {
  ...
  function compile(file) {
    if (!file.startsWith(root + path.sep))
      throw new Error('Module outside project.');
    if (seen.has(file)) return seen.get(file);
    const destination = path.join(
      output,
      path.relative(root, file).replace(/\.ts$/, '.mjs'),
    );
    ...
    code = code.replace(/from\s+['"]([^'"]+)['"]/g, (full, name) => {
      ...
      const source = fs.existsSync(stem + '.ts')
        ? stem + '.ts'
  ...
  return import(
    pathToFileURL(compile(path.join(root, 'lib', 'prospecting.ts'))).href
  );
}
