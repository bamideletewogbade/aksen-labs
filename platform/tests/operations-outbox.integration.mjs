...
const table=readFileSync('db/operations-migration.sql','utf8').replace(/^\uFEFF/,'').split(';')[0].replace('CREATE TABLE IF NOT EXISTS','CREATE TEMP TABLE')+' ON COMMIT DROP';
...
assert.equal(result[2].length,0);assert.equal(result[3].length,1);assert.equal(result[4].length,0);assert.equal(result[6].length,0);
console.log('PASS: temporary outbox owner isolation, one-time claim and no retry after an uncertain send. No real messages or records created.');
