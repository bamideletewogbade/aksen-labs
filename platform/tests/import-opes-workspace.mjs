...
const businesses =
  await q`SELECT id FROM business_workspaces WHERE name=${"Ope's TFS · discovery"}`;
if (businesses.length !== 1)
  throw new Error('Expected exactly one UI-created prospect workspace.');
...
const note =
  'These imported documents preserve the earlier Claude research. They are not client-confirmed. Failure to find an indexed website is not proof that no website exists. A shared phone number does not establish a single legal entity. Follower counts and a single advertised product price do not establish sales, price positioning or budget. Confirm these during discovery before choosing a brand, domain, scope or price. No call outcome, signed contract, invoice or payment is assumed.';
...
console.log(
  'Imported four client source documents and one evidence review note into the existing prospect workspace.',
);
