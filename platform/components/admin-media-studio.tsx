...
const STYLE_PRESETS = [
  {
    label: 'Product shot',
    snippet:
      'clean studio product photography, soft directional light, carbon-black and soft-white surfaces, restrained signal-green accent',
  },
  {
    label: 'Accra lifestyle',
    snippet:
      'authentic contemporary Accra setting, natural daylight, real people, no visual stereotypes',
  },
  {
    label: 'Abstract signal',
    snippet:
      'abstract geometric signal motif, monochrome with a single signal-green accent, no readable text, no logos',
  },
  {
    label: 'Team at work',
    snippet:
      'capable professionals collaborating in a modern office, candid and unposed, premium editorial realism',
  },
];
...
export function AdminMediaStudio() {
  ...
  async function loadGallery() {
    try {
      ...
      /* gallery is a nice-to-have; ignore failures */
    }
  }
  ...
  function applyPreset(snippet: string) {
    setPrompt((current) =>
      current.trim() ? `${current.trim()}, ${snippet}` : snippet,
    );
  }
  ...
  async function enhancePrompt() {
    ...
    setEnhancing(true);
    try {
      const response = await fetch('/api/admin/media/enhance-prompt', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
  ...
  function useResultAsReference() {
    ...
    setReferences((current) => [
      ...current,
      { id: crypto.randomUUID(), dataUrl: result.dataUrl },
    ]);
  }
  ...
  function reuseFromGallery(item: GalleryItem) {
    ...
    setResult(
      item.kind === 'image'
        ? { kind: 'image', dataUrl: item.url }
        : { kind: 'video', url: item.url },
    );
  }
  ...
  async function pollVideoUntilDone(jobId: string) {
    ...
    throw new Error(
      'The video is taking longer than expected. Check back shortly.',
    );
  }
  ...
  async function submit(event: { preventDefault(): void }) {
    ...
    setProgressNote(
      mode === 'video'
        ? 'Starting the video job...'
        : 'Generating the image...',
    );
    try {
      const referenceImages = references.map((ref) => ref.dataUrl);
      if (mode === 'image') {
        const response = await fetch('/api/admin/media/image', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
