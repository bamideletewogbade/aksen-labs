...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const status = body.status === 'resolved' || body.status === 'open' ? body.status : '';
  ...
  await db.insert(auditEvents).values({ id: crypto.randomUUID(), actorId: user.userId, actorType: 'user', action: `conversation.${status}`, entityType: 'conversation', entityId: id }).catch(() => null);
