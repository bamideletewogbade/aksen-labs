...
export async function POST(request: Request) {
  ...
  try {
    ...
    await db.insert(mediaAssets).values({ id: assetId, createdBy: user.userId, kind: 'image', prompt, model: image.model, aspectRatio: aspectRatio || null, url: dataUrl, referenceCount: referenceUrls.length }).catch(() => null);
    await db.insert(auditEvents).values({ id: crypto.randomUUID(), actorId: user.userId, actorType: 'user', action: 'media.image_generated', entityType: 'media_asset', entityId: assetId, details: { model: image.model, referenceCount: referenceUrls.length } }).catch(() => null);
    await logAgentRun({ agentName: 'Creative Studio (image)', channel: 'admin', status: 'success', outcome: prompt.slice(0, 160), durationMs: Date.now() - startedAt, costMicros: image.costMicros });
    return NextResponse.json({ id: assetId, dataUrl, model: image.model });
  } catch {
    await logAgentRun({ agentName: 'Creative Studio (image)', channel: 'admin', status: 'error', outcome: 'Generation failed', durationMs: Date.now() - startedAt });
    return NextResponse.json({ error: 'The image could not be generated right now. Try a simpler prompt or fewer reference images.' }, { status: 502 });
  }
}
