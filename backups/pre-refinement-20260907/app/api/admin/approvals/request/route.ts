...
export async function POST(request: Request) {
  ...
  if (!postId) return NextResponse.json({ error: 'A post id is required.' }, { status: 400 });

  const db = getDb();
  const [post] = await db.select().from(blogPosts).where(eq(blogPosts.id, postId)).limit(1);
  if (!post) return NextResponse.json({ error: 'Post not found.' }, { status: 404 });
  if (post.status === 'published') return NextResponse.json({ error: 'That article is already published.' }, { status: 409 });

  const [existing] = await db.select({ id: approvals.id }).from(approvals)
    .where(and(eq(approvals.entityType, 'blog_post'), eq(approvals.entityId, postId), eq(approvals.status, 'pending')))
    .limit(1);
  if (existing) return NextResponse.json({ id: existing.id, status: 'pending', duplicate: true });
  ...
  await db.insert(auditEvents).values({ id: crypto.randomUUID(), actorId: user.userId, actorType: 'user', action: 'approval.requested', entityType: 'blog_post', entityId: postId, details: { approvalId: id } }).catch(() => null);
