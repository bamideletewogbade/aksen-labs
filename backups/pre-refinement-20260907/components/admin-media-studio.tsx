...
import { Download, ImagePlus, Loader2, Plus, Sparkle, Sparkles, Video, X } from 'lucide-react';
...
export function AdminMediaStudio() {
  ...
  return (
    <section className="admin-panel media-studio-panel">
      <div className="panel-head"><div><small>CREATIVE STUDIO</small><h2>Generate a marketing asset</h2></div><Sparkle /></div>
      ...
      <form className="admin-create-form media-studio-form" onSubmit={submit}>
        ...
        <div className="media-preset-row">
          ...
          <button type="button" className="media-enhance-button" onClick={enhancePrompt} disabled={!prompt.trim() || enhancing}>{enhancing ? <Loader2 size={13} className="icon-spin" /> : <Sparkles size={13} />} Enhance</button>
        </div>
        ...
        <button className="create-submit" disabled={status === 'working' || !prompt.trim()}>{status === 'working' ? <><Loader2 className="icon-spin" /> {progressNote || 'Working...'}</> : <>Generate {mode} <Sparkle /></>}</button>
